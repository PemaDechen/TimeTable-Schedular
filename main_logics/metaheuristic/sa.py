...
# Log file Generation
run_log_path = os.path.join(metrics_dir, "sa_run_log.csv")
...
def simulated_annealing(
    ...
    while temperature > min_temp and iteration < max_iterations:
        ...
        neighbor_cost, neighbor_penalty_breakdown = full_cost_function(
...
def sa_worker(args):
    ...
    # ───── GRAPHING ─────
    cost_folder = os.path.join(metrics_dir, f"sa_cost_vs_iteration_{temp}")
    penalty_folder = os.path.join(metrics_dir, f"sa_penalty_breakdown_{temp}")
...
def sa_main_code():
    ...
    for temp in initial_temps:
        ...
        with open(
            os.path.join(metrics_dir, f"sa_top3_temp_{temp}.csv"),
            "w",
            newline="",
        ) as f:
            writer = csv.writer(f)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(metrics_dir, "sa_combined_cost_vs_iteration.png"))
    plt.close()
