...
def tabu_search_local(solution, tabu_size=50, max_iterations=10):
    ...
    for _ in range(max_iterations):
        ...
        evaluated = [(n, full_cost_function(n, courses, rooms, students, weights, distributions)[0]) for n in neighbors]
...
def hybrid_ga_ts(pop_size=30, generations=50, mutation_rate=0.2, tabu_size=50, enable_output=True, label="HybridGA_TS_FinalResult"):
    ...
    if enable_output:
        plot_cost_versus_iteration_graph(cost_history, title=f"Hybrid GA+TS: Cost vs Generation ({label})")
        final_cost, _, penalty_breakdown = full_cost_function(best_solution, courses, rooms, students, weights, distributions)
        plot_penalty_breakdown(penalty_breakdown, title=f"Hybrid GA+TS: Penalty Breakdown ({label})")
        result(best_solution, label)

    return best_solution, best_cost, cost_history

def ga_ts_worker(run_id, pop_size):
    sol, cost, history = hybrid_ga_ts(pop_size=pop_size, enable_output=False)
...
def run_parallel_hybrid_ga_ts_experiments():
    ...
    with multiprocessing.Pool(processes=min(multiprocessing.cpu_count(), len(configs))) as pool:
        results = pool.starmap(ga_ts_worker, configs)
    df = pd.DataFrame(results)
    os.makedirs("output_files/metrics_track", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    df.to_csv(f"output_files/metrics_track/hybrid_ga_ts_experiment_{timestamp}.csv", index=False)

    plt.figure(figsize=(8, 5))
    sns.boxplot(x='Population Size', y='Best Cost', data=df)
    plt.title("Hybrid GA+TS: Best Cost vs Population Size")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
